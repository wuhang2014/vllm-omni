...
from vllm_omni.engine.omni_core_engine import (
    OmniMasterServer,
    StageAllocation,
...
class TestInitializeStagesRouting:
    ...
    def _run_initialize_stages_mocked(
        ...
        *,
        ...
        mock_launch = Mock(
            side_effect=launch_side_effect or (lambda cfg, meta, spec, timeout, kv: started_by_stage[meta.stage_id])
        )
        mock_remote = Mock(
...
class TestConnectRemoteEngineCoresCoordinator:
    ...
    def test_uses_registered_coordinator_addresses(self):
        ...
        with (
            patch("vllm_omni.engine.omni_core_engine.zmq_socket_ctx", return_value=fake_socket_ctx()),
            patch("vllm_omni.engine.omni_core_engine._wait_for_omni_engine_startup") as mock_wait,
        ):
    ...
    def test_defaults_to_no_coordinator_addresses_when_none_registered(self):
        ...
        with (
            patch("vllm_omni.engine.omni_core_engine.zmq_socket_ctx", return_value=fake_socket_ctx()),
            patch("vllm_omni.engine.omni_core_engine._wait_for_omni_engine_startup"),
        ):
            with connect_remote_engine_cores(
                vllm_config=vllm_config,
                omni_master_server=omni_master_server,
...
class TestLaunchOmniCoreEngines:
    ...
    def test_registers_stage_once_and_reuses_handshake_for_all_local_engines(self):
        ...
        with (
            ...
            patch("vllm_omni.engine.omni_core_engine.zmq_socket_ctx", return_value=fake_socket_ctx()),
            patch(
                "vllm_omni.engine.omni_core_engine.CoreEngineProcManager",
                return_value=local_engine_manager,
            ) as mock_manager_cls,
            patch("vllm_omni.engine.omni_core_engine.wait_for_engine_startup") as mock_wait,
        ):
        ...
        assert manager_kwargs["target_fn"] is EngineCoreProc.run_engine_core
...
class TestLaunchLlmStageSingleStageMode:
    ...
    def _patch_launch_omni_cm(self, stage_id: int):
        ...
        with (
            ...
            patch("vllm_omni.engine.async_omni_engine.get_engine_zmq_addresses"),
            patch("vllm_omni.engine.async_omni_engine.launch_core_engines"),
        ):
    ...
    def test_launch_core_engines_used_in_normal_mode(self):
        """~single_stage_mode → launch_core_engines (not launch_omni_core_engines)."""
        engine = object.__new__(AsyncOmniEngine)
        engine.model = "fake-model"
        ...
        @contextmanager
        def fake_launch_core(*args, **kwargs):
            yield eng_mgr, None, fake_addresses
        ...
        with (
            ...
            patch("vllm_omni.engine.async_omni_engine.get_engine_zmq_addresses", return_value=fake_addresses),
            patch(
                "vllm_omni.engine.async_omni_engine.launch_core_engines", return_value=fake_launch_core()
            ) as mock_lce,
    ...
    def test_launch_omni_passes_stage_id_and_master_server(self):
        ...
        with (
            ...
            patch("vllm_omni.engine.async_omni_engine.acquire_device_locks", return_value=[]),
            patch("vllm_omni.engine.async_omni_engine.release_device_locks"),
            patch("vllm_omni.engine.async_omni_engine.launch_omni_core_engines", return_value=capturing_launch()),
            patch("vllm_omni.engine.async_omni_engine.get_engine_zmq_addresses"),
            patch("vllm_omni.engine.async_omni_engine.launch_core_engines"),
        ):
